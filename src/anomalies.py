...
def check_oil_temperature(
    ...
    temp_col = df.columns[
        0
    ]  # Assuming the oil temperature column is the first one in the DataFrame
    ...
    df["block_id"] = df["above_threshold"].ne(df["above_threshold"].shift()).cumsum()
            ...
            flagged_periods.append(
                {
                    ...
                    "metrics": {
                        "average_temperature": round(float(group[temp_col].mean()), 2),
                        "maximum_temperature": round(float(group[temp_col].max()), 2),
                    },
                }
            )
...
def detect_anomalies(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for comp_id in compressors:
        comp_df = df[
            [f"compressor_{comp_id}_oil_temp", f"compressor_{comp_id}_activity"]
        ].copy()
        ...
        anomalies[f"compressor_{comp_id}"] = {
            ...
            "activity_transition_anomalies": (
                activity_check if comp_id in [1, 2] else None
            ),
        }
